...
// Temporary admin role assignment utility
Route::get('/admin-assign/{id?}', function ($id = null) {
    $userId = $id ?: auth()->id();
    
    if (!$userId) {
        return 'No user specified and you are not logged in';
    }
    ...
    if (!$hasRole) {
        ...
        // Also set direct role attribute for compatibility
        $user->update(['role' => 'admin']);
        
        $status = 'Admin role successfully assigned to user ' . $user->firstname . ' ' . $user->lastname;
    } else {
        $status = 'User already has admin role';
    }
    
    return view('admin.utils.role-assignment', [
        'status' => $status,
