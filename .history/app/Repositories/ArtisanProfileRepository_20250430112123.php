...
class ArtisanProfileRepository implements ArtisanProfileRepositoryInterface
{
    ...
    /**
     * Get filtered artisans for client search
     *
     * @param array $filters
     * @return \Illuminate\Pagination\LengthAwarePaginator
     */
    public function getClientFilteredArtisans(array $filters = []): \Illuminate\Pagination\LengthAwarePaginator
    {
        // Create base query for artisan profiles
        $query = ArtisanProfile::with(['user', 'categories', 'services', 'reviews'])
        ...
        // Get page size from filters or default to 12
    ...
    /**
     * Get all distinct artisan locations
     *
     * @return \Illuminate\Support\Collection
     */
    public function getDistinctLocations(): \Illuminate\Support\Collection
    {
        return ArtisanProfile::select('city')
            ->whereNotNull('city')
            ->where('city', '!=', '')
            ->distinct()
            ->orderBy('city')
    ...
    /**
     * Get artisan detail with all related information
     *
     * @param int $artisanProfileId
     * @return ArtisanProfile|null
     */
    public function getArtisanDetail(int $artisanProfileId): ?ArtisanProfile
    {
        try {
            return ArtisanProfile::with([
                'user',
                'categories',
            ...
            ])->findOrFail($artisanProfileId);
        } catch (\Exception $e) {
            Log::error("Error retrieving artisan detail: " . $e->getMessage());
            return null;
        }
    }
}
