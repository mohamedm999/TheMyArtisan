...
class ArtisanProfileRepository implements ArtisanProfileRepositoryInterface
{
    ...
    public function getArtisanStatistics(): array
    {
        return [
            ...
            'pending' => User::whereHas('roles', function ($query) {
                ...
                $q->where('status', ArtisanProfile::STATUS_PENDING);
            })->count(),
            'approved' => User::whereHas('roles', function ($query) {
                $query->where('name', 'artisan');
            })->whereHas('artisanProfile', function ($q) {
                $q->where('status', ArtisanProfile::STATUS_APPROVED);
            })->count(),
            'rejected' => User::whereHas('roles', function ($query) {
                $query->where('name', 'artisan');
            })->whereHas('artisanProfile', function ($q) {
                $q->where('status', ArtisanProfile::STATUS_REJECTED);
            })->count(),
        ];
    }
    ...
    public function createMissingArtisanProfiles(): int
    {
        ...
        $artisanUsers = User::whereHas('roles', function ($query) {
            $query->where('name', 'artisan');
        })->whereDoesntHave('artisanProfile')
          ->get();

        foreach ($artisanUsers as $user) {
            ArtisanProfile::create([
                'user_id' => $user->id,
                'status' => ArtisanProfile::STATUS_PENDING,
            ]);
            $created++;
        }
    ...
    public function updateArtisanStatus(int $userId, string $status): ?ArtisanProfile
    {
        try {
            ...
            if (!$profile) {
                Log::warning('Attempted to update status for non-existent artisan profile.', ['user_id' => $userId]);
                return null;
            }

            if (!in_array($status, [ArtisanProfile::STATUS_PENDING, ArtisanProfile::STATUS_APPROVED, ArtisanProfile::STATUS_REJECTED])) {
                Log::error('Invalid status provided for artisan profile update.', ['user_id' => $userId, 'status' => $status]);
                return null;
            }
            ...
            Log::info('Artisan profile status updated.', ['user_id' => $userId, 'status' => $status]);
            return $profile;
        } catch (\Exception $e) {
