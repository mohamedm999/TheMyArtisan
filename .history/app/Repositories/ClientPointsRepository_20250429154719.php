...
class ClientPointsRepository implements ClientPointsRepositoryInterface
{
    ...
    public function getPointsAnalytics(): array
    {
        ...
        $today = PointTransaction::where('type', PointTransaction::TYPE_EARNED)
            ->whereDate('created_at', Carbon::today())
            ->sum('points');

        // Points awarded this week
        $thisWeek = PointTransaction::where('type', PointTransaction::TYPE_EARNED)
            ->whereBetween('created_at', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()])
            ->sum('points');

        // Points awarded this month
        $thisMonth = PointTransaction::where('type', PointTransaction::TYPE_EARNED)
            ->whereYear('created_at', Carbon::now()->year)
            ->whereMonth('created_at', Carbon::now()->month)
            ->sum('points');

        // Total points ever awarded
        $totalAwarded = PointTransaction::where('type', PointTransaction::TYPE_EARNED)
            ->sum('points');

        // Points redeemed today
        $todayRedeemed = PointTransaction::where('type', PointTransaction::TYPE_REDEEMED)
            ->whereDate('created_at', Carbon::today())
            ->sum('points');

        // Points redeemed this month
        $monthRedeemed = PointTransaction::where('type', PointTransaction::TYPE_REDEEMED)
            ->whereYear('created_at', Carbon::now()->year)
            ->whereMonth('created_at', Carbon::now()->month)
            ->sum('points');

        // Total points redeemed
        $totalRedeemed = PointTransaction::where('type', PointTransaction::TYPE_REDEEMED)
            ->sum('points');

        // Points by source
        $pointsBySource = PointTransaction::where('type', PointTransaction::TYPE_EARNED)
