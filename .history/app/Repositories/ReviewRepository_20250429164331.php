...
class ReviewRepository implements ReviewRepositoryInterface
{
    ...
    {
        $query = Review::with(['client', 'artisan', 'service', 'clientProfile', 'artisanProfile']);
        ...
        if (!empty($filters['search'])) {
            ...
            $query->where(function($q) use ($search) {
                $q->where('comment', 'like', '%' . $search . '%')
                  ...
                  ->orWhereHas('artisan', function($sq) use ($search) {
    ...
    {
        return Review::with(['client', 'artisan', 'booking', 'service', 'clientProfile', 'artisanProfile'])
            ->findOrFail($id);
    }
}
