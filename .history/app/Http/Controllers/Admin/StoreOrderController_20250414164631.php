...
class StoreOrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Clone the base query for statistical calculations
        $statsQuery = clone $baseQuery;
        
        // Calculate statistics based on the current filter
        ...
        $totalPointsSpent = (clone $statsQuery)->sum('points_spent');

        // Apply sorting and get paginated results for display
        $orders = $baseQuery->with(['user', 'product'])
            ->orderBy('created_at', 'desc')
            ->paginate(15)
            ->withQueryString(); // Preserve the query parameters
