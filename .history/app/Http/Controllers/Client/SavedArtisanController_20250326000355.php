...
class SavedArtisanController extends Controller
{
    ...
    public function index()
    {
        ...
        // Method 1: Get through the pivot table first to ensure we have the correct IDs
        $savedArtisanIds = SavedArtisan::where('user_id', $userId)
            ->pluck('artisan_profile_id')
            ->toArray();

        Log::info('Found saved artisan IDs: ' . implode(', ', $savedArtisanIds ?: ['none']));

        // Then retrieve the full artisan profiles with their relationships
