...
class FindArtisansController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $availability = $request->input('availability', 'all'); // New availability filter
            ...
            // Only filter by status if the column exists in the table
            if (Schema::hasColumn('artisan_profiles', 'status')) {
                $query->where('status', 'approved');
            } else {
                // Log that the status column is missing
                Log::warning("The 'status' column does not exist in the artisan_profiles table. Skipping status filter.");
            }
            ...
            return view('client.find-artisans', compact('artisans', 'categories', 'search', 'category', 'sortBy', 'location', 'availability'));
        } catch (\Exception $e) {
            ...
            return view('client.find-artisans', [
                // Create a proper paginator instance with empty data
