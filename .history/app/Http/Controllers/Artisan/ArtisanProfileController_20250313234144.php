...
class ArtisanProfileController extends Controller
{
    public function index()
    {
        ...
        // Get fresh data every time to avoid caching issues
        $workExperiences = WorkExperience::where('user_id', Auth::id())
            ->orderBy('created_at', 'desc')
            ->get();

        \Log::info('Loaded work experiences for profile page:', [
    ...
    public function storeWorkExperience(Request $request)
    {
        ...
        \Log::info('Work Experience Data Received:', $request->all());

        $result = $this->addWorkExperience($request);

        // Store the newly created work experience in the session for debugging
    ...
    /**
     * Debug method to check work experiences
     */
    public function debugWorkExperience()
    {
        $workExperiences = WorkExperience::where('user_id', Auth::id())->get();

        echo "<h1>Work Experience Debug</h1>";
        echo "<pre>";
        echo "User ID: " . Auth::id() . "\n";
        echo "Count: " . $workExperiences->count() . "\n\n";
        print_r($workExperiences->toArray());
        echo "</pre>";
        die();
    }
}
