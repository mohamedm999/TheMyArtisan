...
class ScheduleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $artisanId = auth()->id();
        $availabilities = Availability::where('artisan_id', $artisanId)
            ->whereBetween('date', [$startDate->format('Y-m-d'), $endDate->format('Y-m-d')])
            ->get()
            ->groupBy('date');

        // Format each availability for display
        $formattedAvailabilities = [];
        foreach ($availabilities as $date => $dayAvailabilities) {
            $formattedAvailabilities[$date] = $dayAvailabilities->map(function ($item) {
                return [
                    'id' => $item->id,
    ...
    public function store(Request $request)
    {
        ...
        $overlapping = Availability::where('artisan_id', $artisanId)
