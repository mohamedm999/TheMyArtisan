...
class ReviewsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $reviewsQuery = Review::where('artisan_id', $user->id)
            ->with(['customer', 'service', 'booking'])
        ...
        $stats = [
            'total' => Review::where('artisan_id', $user->id)->count(),
            'average_rating' => Review::where('artisan_id', $user->id)->avg('rating') ?? 0,
            'five_stars' => Review::where('artisan_id', $user->id)->where('rating', 5)->count(),
            'four_stars' => Review::where('artisan_id', $user->id)->where('rating', 4)->count(),
            'three_stars' => Review::where('artisan_id', $user->id)->where('rating', 3)->count(),
            'two_stars' => Review::where('artisan_id', $user->id)->where('rating', 2)->count(),
            'one_star' => Review::where('artisan_id', $user->id)->where('rating', 1)->count(),
            'pending_response' => Review::where('artisan_id', $user->id)->whereNull('response')->count(),
        ];
