...
<nav class="bg-white/95 backdrop-blur-sm shadow-md border-b border-gray-100/40 sticky top-0 z-30" x-data="{ mobileMenuOpen: false }">
        ...
        <div class="flex justify-between h-16">
            ...
            <div class="flex">
                ...
                <div class="hidden sm:ml-8 sm:flex sm:items-center sm:space-x-8">
                    ...
                    <!-- Enhanced dropdown with better interactions and fixed alignment -->
    ...
    @auth
        <div class="fixed bottom-0 right-0 bg-white p-2 text-xs border border-gray-300 m-2 z-50">
            <strong>User ID:</strong> {{ Auth::user()->id }} <br>
            <strong>Direct role attribute:</strong> {{ Auth::user()->role ?? 'Not set' }} <br>
            <strong>Has admin role (method):</strong> {{ Auth::user()->hasRole('admin') ? 'Yes' : 'No' }} <br>
            <strong>Roles from relationship:</strong> {{ implode(', ', Auth::user()->roles->pluck('name')->toArray()) ?: 'None' }} <br>
            <strong>Role check from database:</strong> {{ Auth::user()->roles()->where('name', 'admin')->exists() ? 'Admin found' : 'No admin role' }}
        </div>
    @endauth
</nav>
