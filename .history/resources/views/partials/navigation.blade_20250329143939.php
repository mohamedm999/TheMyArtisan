...
<nav class="bg-white/95 backdrop-blur-sm shadow-md border-b border-gray-100/40 sticky top-0 z-30" x-data="{ mobileMenuOpen: false }">
    ...
    <!-- Updated debug section with more detailed role information -->
    @auth
        <div class="fixed bottom-0 right-0 bg-white p-2 text-xs border border-gray-300 m-2 z-50">
            Role attribute: {{ Auth::user()->role }} <br>
            Has admin role (method): {{ Auth::user()->hasRole('admin') ? 'Yes' : 'No' }} <br>
            Roles: {{ implode(', ', Auth::user()->roles()->pluck('name')->toArray()) }}
        </div>
    @endauth
</nav>
