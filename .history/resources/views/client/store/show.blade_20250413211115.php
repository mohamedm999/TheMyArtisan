    ...
    <div class="container mx-auto px-4 py-4">
        ...
        <div class="flex flex-wrap -mx-4">
            ...
            <div class="w-full lg:w-1/4 px-4">
                ...
                <div class="bg-white rounded-lg shadow mb-4">
                    <div class="px-4 py-3 bg-gray-100 border-b">
                        <h5 class="font-medium">How to Redeem</h5>
                    </div>
                    <div class="p-4">
                        <ol class="pl-5 list-decimal mb-0">
                    ...
                    @foreach ($relatedProducts as $related)
                        <div class="w-full md:w-1/4 px-4 mb-4">
                            <div class="bg-white rounded-lg shadow h-full flex flex-col">
                                @if ($related->image)
                                    <img src="{{ asset('storage/' . $related->image) }}" class="w-full h-40 object-cover rounded-t-lg"
                                        alt="{{ $related->name }}">
                                @else
                                ...
                                <div class="p-4 flex flex-col flex-grow">
                                    <h5 class="text-sm font-semibold">{{ $related->name }}</h5>
                                    <p class="text-xs text-gray-500">{{ \Str::limit($related->description, 50) }}</p>
                                    <div class="flex justify-between items-center mt-auto pt-3">
                                        <span class="px-2 py-1 bg-blue-600 text-white text-xs rounded">
                                            {{ number_format($related->points_cost) }} points
                                        </span>
                                        <a href="{{ route('client.store.product', $related->id) }}"
                                            class="px-3 py-1 text-xs border border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white rounded">
                                            View
                                        </a>
                                    </div>
                                </div>
