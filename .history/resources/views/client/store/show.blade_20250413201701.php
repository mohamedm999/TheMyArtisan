...
<div class="container py-4">
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb bg-light">
            ...
            @if($product->category)
            <li class="breadcrumb-item"><a href="{{ route('client.store.category', $product->category) }}">{{ $product->category }}</a></li>
            @endif
                ...
                <div class="card-body p-0">
                    <div class="row g-0">
                        <div class="col-md-5 border-right">
                            @if($product->image)
                            <img src="{{ asset('storage/' . $product->image) }}" class="img-fluid rounded-start" alt="{{ $product->name }}" style="width: 100%; height: 100%; object-fit: cover;">
                            @else
                            ...
                            <div class="p-4">
                                ...
                                <div class="d-flex align-items-center mb-3">
                                    <div class="badge badge-primary p-2">
                                        <i class="fas fa-coins mr-1"></i> {{ number_format($product->points_cost) }} points
                                    </div>
                                    @if($product->stock !== -1)
                                ...
                                <form action="{{ route('client.store.purchase', $product->id) }}" method="POST">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="number" name="quantity" id="quantity" class="form-control" value="1" min="1" max="{{ $product->stock == -1 ? 10 : min($product->stock, 10) }}">
                                    </div>
                                    
                                    <div class="form-group">
                                        <label for="delivery_details">Delivery Details</label>
                                        <textarea name="delivery_details" id="delivery_details" class="form-control" rows="3" placeholder="Enter your shipping address or any special instructions" required></textarea>
                                    </div>
                                    
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div>
                                            <button type="submit" class="btn btn-primary" {{ !$canAfford ? 'disabled' : '' }}>
                                                <i class="fas fa-shopping-cart mr-1"></i> Redeem Now
                                            </button>
                                        </div>
                                        
                                        <div class="text-right">
                                            <p class="mb-0">Your Balance: <strong>{{ number_format($points) }} points</strong></p>
                                            @if(!$canAfford)
            ...
            <div class="card shadow-sm mb-4">
                ...
                <div class="card-body">
                    <div class="text-center">
                        ...
                        <a href="{{ route('client.points.index') }}" class="btn btn-sm btn-outline-primary">View Points History</a>
                    </div>
                </div>
            </div>
                ...
                <div class="card h-100 shadow-sm">
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-auto">
                            <span class="badge badge-primary">{{ number_format($related->points_cost) }} points</span>
                            <a href="{{ route('client.store.product', $related->id) }}" class="btn btn-sm btn-outline-primary">View</a>
                        </div>
                    </div>
                </div>
            </div>
