            ...
            <div class="bg-white shadow-lg rounded-lg overflow-hidden border border-gray-100">
                ...
                @if(isset($savedArtisans) && count($savedArtisans) > 0)
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
                        @foreach($savedArtisans as $artisan)
                            <div class="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200 transition-all duration-200 hover:shadow-md hover:border-green-300">
                                <div class="p-5">
                                    <div class="flex items-center mb-4">
                                        ...
                                        @else
                                            <div class="h-14 w-14 rounded-full bg-gradient-to-r from-green-500 to-blue-600 flex items-center justify-center mr-3 shadow-sm">
                                        ...
                                        @endif
                                        <div>
                                            <h3 class="font-semibold text-gray-800 text-lg">{{ $artisan->user->firstname }} {{ $artisan->user->lastname }}</h3>
                                            <p class="text-sm text-gray-500">{{ $artisan->primary_service }}</p>
                                        </div>
                                        <button class="ml-auto text-red-500 hover:text-red-700 focus:outline-none"
                                                onclick="event.preventDefault(); document.getElementById('unsave-form-{{ $artisan->id }}').submit();">
                                        ...
                                        <form id="unsave-form-{{ $artisan->id }}" action="{{ route('client.unsave-artisan', $artisan->id) }}" method="POST" class="hidden">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </div>
                                    <div class="mb-4 space-y-2">
                                        ...
                                        <div class="flex items-center text-sm">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-yellow-500 mr-2" fill="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <div class="flex space-x-2">
                                        ...
                                        <a href="{{ route('client.bookings.create', ['artisan_id' => $artisan->id]) }}"
                ...
                @else
                    ...
                    <div class="text-center py-16 px-4">
                        ...
                        <p class="mt-2 text-base text-gray-600 max-w-md mx-auto">When you find artisans you like, save them here for quick access and easy booking.</p>
                        <div class="mt-8">
                            <a href="{{ route('client.find-artisans') }}"
