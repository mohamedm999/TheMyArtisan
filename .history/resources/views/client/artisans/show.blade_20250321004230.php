            ...
            <div class="lg:flex lg:items-center lg:justify-between">
                <div class="flex-1 min-w-0">
                    <div class="flex items-center">
                        ...
                        @else
                            <div class="w-20 h-20 md:w-24 md:h-24 bg-gradient-to-r from-green-400 to-blue-500 rounded-full flex justify-center items-center border-4 border-white shadow-lg">
                        ...
                        <div class="ml-4">
                            ...
                            <div class="flex items-center mt-2">
                                ...
                                <span class="text-xs md:text-sm text-gray-600 ml-2">({{ $artisan->reviews_count }} reviews)</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-5 flex flex-col sm:flex-row sm:flex-wrap sm:mt-0 sm:space-x-6">
                        <div class="mt-2 flex items-center text-sm text-gray-500">
                            <svg class="flex-shrink-0 mr-1.5 h-5 w-5 text-green-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                ...
                <div class="mt-5 flex lg:mt-0 lg:ml-4 flex-col sm:flex-row gap-3">
                    <a href="#contact" class="sm:inline-flex sm:items-center px-5 py-2 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                        Contact Me
                    </a>
                    <a href="#" onclick="switchTab(event, 'services')" class="sm:inline-flex sm:items-center px-5 py-2 border border-gray-300 text-base font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                        View Services
                    </a>
                </div>
            </div>
    ...
    <div class="bg-white shadow">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex overflow-x-auto py-3 space-x-8">
                <button onclick="switchTab(event, 'about')" class="tab-btn active whitespace-nowrap py-2 px-1 text-gray-500 hover:text-gray-900">About</button>
                <button onclick="switchTab(event, 'services')" class="tab-btn whitespace-nowrap py-2 px-1 text-gray-500 hover:text-gray-900">Services</button>
                <button onclick="switchTab(event, 'experience')" class="tab-btn whitespace-nowrap py-2 px-1 text-gray-500 hover:text-gray-900">Experience</button>
                <button onclick="switchTab(event, 'certifications')" class="tab-btn whitespace-nowrap py-2 px-1 text-gray-500 hover:text-gray-900">Certifications</button>
                <button onclick="switchTab(event, 'availability')" class="tab-btn whitespace-nowrap py-2 px-1 text-gray-500 hover:text-gray-900">Availability</button>
                <button onclick="switchTab(event, 'reviews')" class="tab-btn whitespace-nowrap py-2 px-1 text-gray-500 hover:text-gray-900">Reviews</button>
                <button onclick="switchTab(event, 'contact')" class="tab-btn whitespace-nowrap py-2 px-1 text-gray-500 hover:text-gray-900">Contact</button>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div id="services" class="tab-content">
            <div class="mb-6">
                ...
                <p class="text-gray-500 text-sm mt-1">Browse through {{ $artisan->user->firstname }}'s professional services</p>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($artisan->services as $service)
                    <div class="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-100 transition transform hover:-translate-y-1 hover:shadow-md">
                        @if ($service->image)
                            <img src="{{ asset('storage/' . $service->image) }}" class="w-full h-48 object-cover" alt="{{ $service->name }}">
                        @else
                            <div class="w-full h-32 bg-gradient-to-r from-green-200 to-blue-200 flex items-center justify-center">
                                <svg class="w-12 h-12 text-white opacity-75" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                    <path d="M12 6a6 6 0 00-6 6c0 1.8.8 3.4 2 4.5V18a2 2 0 002 2h4a2 2 0 002-2v-1.5c1.2-1 2-2.7 2-4.5a6 6 0 00-6-6zm2 12h-4v-1h4v1zm.5-3.5l-.5.5v1h-4v-1l-.5-.5A4 4 0 018 12a4 4 0 118 0c0 1.5-.8 2.8-2 3.5zM14 9h1v3h-1V9zm-5 0h1v3H9V9z"/>
                                </svg>
                            </div>
                        @endif
                        <div class="p-5">
                            <div class="flex justify-between items-start">
                                ...
                                <span class="px-2 py-1 bg-green-100 text-green-800 text-xs font-semibold rounded-full">{{ number_format($service->price, 2) }} DH</span>
                            </div>
                            ...
                            <div class="mt-4 flex justify-between items-center">
                                ...
                                <a href="{{ route('client.services.show', $service->id) }}" class="inline-flex items-center px-3 py-1 text-sm bg-white border border-green-600 text-green-600 rounded-md hover:bg-green-50">
                                    View Details
                            ...
                            <div class="flex items-start">
                                ...
                                <div class="ml-4">
                                    ...
                                    @if ($certification->valid_until)
                                        <p class="text-sm text-gray-500 mt-1">Valid until: {{ $certification->valid_until->format('M Y') }}</p>
                                    @endif
                                    @if ($certification->description)
                                        <p class="mt-3 text-gray-600 text-sm">{{ $certification->description }}</p>
                                    @endif
                                    @if ($certification->credential_url)
                                        <a href="{{ $certification->credential_url }}" target="_blank" class="mt-2 inline-flex items-center text-sm text-blue-600 hover:text-blue-800">
                    ...
                    @if (count($artisan->availabilities) > 0)
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            @foreach ($artisan->availabilities->sortBy('date')->take(6) as $availability)
                                <div class="bg-white rounded-md border {{ $availability->status === 'booked' ? 'border-red-200 bg-red-50' : 'border-green-200 bg-green-50' }} p-3">
                                    <div class="font-medium text-gray-900">{{ $availability->date->format('D, M d, Y') }}</div>
                                    <div class="text-sm text-gray-700 mt-1">
                                        {{ $availability->start_time->format('g:i A') }} - {{ $availability->end_time->format('g:i A') }}
                                    </div>
                                    <div class="mt-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $availability->status === 'booked' ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800' }}">
        ...
        <div id="reviews" class="tab-content">
            <div class="mb-6">
                ...
                <p class="text-gray-500 text-sm mt-1">What others say about working with {{ $artisan->user->firstname }}</p>
            </div>
                            ...
                            <div class="flex justify-between items-start mb-4">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 w-10 h-10 bg-gray-200 rounded-full flex items-center justify-center">
                                        <span class="text-gray-500 font-medium">{{ substr($review->user->firstname ?? 'A', 0, 1) }}</span>
                                    </div>
                                    <div class="ml-3">
                                        <h3 class="font-medium text-gray-900">{{ $review->user->firstname ?? 'Anonymous' }}</h3>
            ...
            <div class="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-100">
                <div class="p-5">
                    ...
                    <form action="{{ route('client.contact') }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            <label for="schedule_date" class="block text-sm font-medium text-gray-700">Schedule a Meeting (optional)</label>
                            <div class="mt-1">
