                                ...
                                <div class="h-48 bg-gray-200 relative">
                                    @if ($artisan->artisanProfile && $artisan->artisanProfile->profile_photo)
                                        <img src="{{ asset('storage/' . $artisan->artisanProfile->profile_photo) }}"
                                            alt="{{ $artisan->firstname }} {{ $artisan->lastname }}" class="w-full h-full object-cover">
                                    @else
                                        <div
                                    ...
                                    <div
                                        ...
                                        <h3 class="font-semibold text-lg">{{ $artisan->firstname }} {{ $artisan->lastname }}</h3>
                                ...
                                @if ($artisan->artisanProfile && isset($artisan->artisanProfile->services) && $artisan->artisanProfile->services->count() > 0)
                                    <div class="mt-2 mb-3">
                                        ...
                                        <div class="flex flex-wrap gap-1">
                                            @foreach ($artisan->artisanProfile->services->take(3) as $service)
                                                <span class="text-xs bg-amber-50 text-amber-700 px-2 py-1 rounded">
                                                    {{ $service->name }}
                                                </span>
                                            @endforeach
                                            @if ($artisan->artisanProfile->services->count() > 3)
                                                <span class="text-xs text-gray-500">+{{ $artisan->artisanProfile->services->count() - 3 }} more</span>
                                            @endif
                                        </div>
                                    </div>
                                @endif
