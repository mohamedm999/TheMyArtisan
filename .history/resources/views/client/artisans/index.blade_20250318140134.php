...
@section('content')
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <!-- Detailed Debug Info - Remove after fixing -->
            @if (app()->environment('local'))
                <div class="bg-blue-100 p-4 mb-6 rounded overflow-auto">
                    <h3 class="font-bold text-blue-800 mb-2">Debug Information:</h3>
                    @if (isset($debugInfo))
                        <ul class="list-disc pl-5 mb-2 text-blue-800 text-sm">
                            <li>Artisan Role: {{ $debugInfo['artisan_role'] ? $debugInfo['artisan_role']->name : 'Not found' }}</li>
                            <li>Artisan Profiles Count: {{ $debugInfo['artisan_profiles_count'] }}</li>
                            <li>Users Count: {{ $debugInfo['users_count'] }}</li>
                            <li>Role Users Count: {{ $debugInfo['role_users'] }}</li>
                        </ul>
                        <p class="text-xs text-blue-800 mt-2"><strong>SQL Query:</strong> {{ $sqlQuery ?? 'N/A' }}</p>
                        <p class="text-xs text-blue-800"><strong>Bindings:</strong> {{ json_encode($bindings ?? []) }}</p>
                    @endif
                    <p class="text-xs text-blue-800 mt-2">Artisans found: {{ isset($artisans) ? $artisans->count() : 'N/A' }}</p>
                </div>
            @endif
