        ...
        <div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
            <div class="lg:flex lg:items-center lg:justify-between">
                ...
                <div class="mt-5 flex lg:mt-0 lg:ml-4 flex-col sm:flex-row gap-3">
                    ...
                    @auth
                        @php
                            $isSaved = Auth::user()->savedArtisans->contains($artisan->id);
                        @endphp

                        @if($isSaved)
                            <form action="{{ route('client.unsave-artisan', $artisan->id) }}" method="POST" id="save-artisan-form">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                        ...
                        @else
                            <form action="{{ route('client.save-artisan', $artisan->id) }}" method="POST" id="save-artisan-form">
                                @csrf
                                <button type="submit"
                ...
                saveForm.addEventListener('submit', function(e) {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            toast.innerHTML = data.message || (isUnsave ? 'Artisan removed from saved list' : 'Artisan saved successfully');
