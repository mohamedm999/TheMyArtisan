            ...
            <form action="{{ route('client.artisans.index') }}" method="GET" class="flex flex-col md:flex-row gap-4">
                <div class="flex-1">
                    ...
                    <div class="relative rounded-md shadow-sm">
                        ...
                        <input type="text" name="search" id="search" class="focus:ring-amber-500 focus:border-amber-500 block w-full pl-10 sm:text-sm border-gray-300 rounded-md" placeholder="Search by name or skill">
                    </div>
                </div>
            ...
            @if($artisans->count() > 0)
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($artisans as $artisan)
                        <a href="{{ route('client.artisans.show', $artisan->id) }}" class="block bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow overflow-hidden border border-gray-100">
                            <div class="bg-gray-200 h-48 overflow-hidden relative">
                                @if($artisan->profile_image)
                                    <img src="{{ asset('storage/' . $artisan->profile_image) }}" alt="{{ $artisan->user->name }}" class="w-full h-full object-cover">
                                @else
                                ...
                                @if($artisan->is_featured)
                                    <div class="absolute top-2 right-2 bg-amber-500 text-white px-2 py-1 rounded-md text-xs font-semibold">
                                        Featured
                                    </div>
                                @endif
                            </div>
                            <div class="p-4">
                                <div class="flex items-start justify-between">
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <span class="font-medium">{{ number_format($artisan->reviews->avg('rating') ?? 0, 1) }}</span>
                                ...
                                <div class="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center">
                                    ...
                                    <span class="text-amber-600 text-sm font-medium hover:text-amber-800">View Profile</span>
                                </div>
                            </div>
                        </a>
                    @endforeach
