        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div class="p-6 md:w-2/3">
                        ...
                        <div class="grid md:grid-cols-2 gap-4 mt-6">
                            @if ($artisan->artisanProfile->profession)
                                <div>
                                    <p class="text-gray-700"><i class="fas fa-tools mr-2 text-amber-600"></i>
                            ...
                            @if ($artisan->artisanProfile->experience_years)
                                <div>
                                    <p class="text-gray-700"><i class="fas fa-briefcase mr-2 text-amber-600"></i>
                                        {{ $artisan->artisanProfile->experience_years }} years of experience</p>
                                </div>
                            @endif

                            @if ($artisan->artisanProfile->city)
                                <div>
                                    <p class="text-gray-700"><i class="fas fa-map-marker-alt mr-2 text-amber-600"></i>
                                        {{ $artisan->artisanProfile->city }}</p>
                                </div>
                            @endif

                            @if ($artisan->artisanProfile->hourly_rate)
                                <div>
                                    <p class="text-gray-700"><i class="fas fa-euro-sign mr-2 text-amber-600"></i> Starting
                                        at €{{ $artisan->artisanProfile->hourly_rate }}/hour</p>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="mt-8">
                            <a href="#book-service"
                                class="inline-block px-6 py-3 bg-amber-600 text-white rounded-md hover:bg-amber-700 transition-colors mr-4">
                                Book a Service
                            </a>
                            <a href="#contact"
                                class="inline-block px-6 py-3 bg-white border border-amber-600 text-amber-600 rounded-md hover:bg-amber-50 transition-colors">
                                Contact
                            </a>
                        </div>
                    </div>
            ...
            <div id="about" class="tab-content bg-white rounded-lg shadow-md p-6 mb-8">
                ...
                <div class="mt-8 grid md:grid-cols-2 gap-6">
                        ...
                        @if ($artisan->artisanProfile->skills && is_array($artisan->artisanProfile->skills))
                            <div class="flex flex-wrap gap-2">
                                @foreach ($artisan->artisanProfile->skills as $skill)
                                    <span
                                        class="bg-amber-100 text-amber-800 text-xs px-3 py-1 rounded-full">{{ $skill }}</span>
                                @endforeach
                            </div>
                        @else
                    ...
                    <div>
                        ...
                        <div class="space-y-2">
                            @if ($artisan->email)
                                <p class="text-gray-700"><i class="fas fa-envelope mr-2 text-amber-600"></i>
                                    {{ $artisan->email }}</p>
                            @endif

                            @if ($artisan->artisanProfile->phone)
                                <p class="text-gray-700"><i class="fas fa-phone mr-2 text-amber-600"></i>
                                    {{ $artisan->artisanProfile->phone }}</p>
                            @endif

                            @if ($artisan->artisanProfile->fullAddress)
                                <p class="text-gray-700"><i class="fas fa-map-marker-alt mr-2 text-amber-600"></i>
                                    {{ $artisan->artisanProfile->fullAddress }}</p>
                            @endif
                        </div>
                    </div>
                </div>
                            ...
                            <div class="border border-gray-200 rounded-lg overflow-hidden">
                                ...
                                <div class="p-4">
                                    ...
                                    <div class="flex justify-between items-center">
                                        ...
                                        <button
                                            class="px-4 py-2 bg-amber-600 text-white text-sm rounded hover:bg-amber-700 transition-colors book-service-btn"
            ...
            <div id="experience" class="tab-content bg-white rounded-lg shadow-md p-6 mb-8 hidden">
                ...
                <div class="mb-8">
                    ...
                    @if (isset($artisan->artisanProfile->workExperiences) && $artisan->artisanProfile->workExperiences->count() > 0)
                        <div class="space-y-6 relative">
                            @foreach ($artisan->artisanProfile->workExperiences as $experience)
                                <div class="relative pl-8 pb-6 border-l-2 border-amber-200 last:border-0 last:pb-0">
                                    ...
                                    <div class="mb-1">
                                        <h4 class="text-lg font-medium text-gray-900">{{ $experience->position ?? $experience->title }}</h4>
                                        <p class="text-amber-600">{{ $experience->company_name }}</p>
                                    </div>
                                    <div class="text-sm text-gray-600 mb-3">
                                        ...
                                        @if ($experience->start_date)
                                            ({{ $experience->is_current
                                                ? $experience->start_date->diffInYears(now())
                                                : ($experience->end_date
                                                    ? $experience->start_date->diffInYears($experience->end_date)
                                                    : 'N/A') }} years)
                                        @endif
                                    </div>
                ...
                <div>
                    ...
                    @if (isset($artisan->artisanProfile->certifications) && $artisan->artisanProfile->certifications->count() > 0)
                        <div class="grid md:grid-cols-2 gap-4">
                            @foreach ($artisan->artisanProfile->certifications as $cert)
                                <div class="border border-gray-200 rounded-lg p-4">
                                    <h4 class="font-medium text-gray-900">{{ $cert->name }}</h4>
                                    <p class="text-gray-600">{{ $cert->issuer }}</p>
                                    @if ($cert->valid_until)
                                        <p class="text-sm text-amber-600">
                                            {{ $cert->valid_until->isFuture() ? 'Expires' : 'Expired' }}:
                                            {{ $cert->valid_until->format('M Y') }}
                                        </p>
                                    @endif
                                </div>
                            @endforeach
                                ...
                                <div class="ml-14">
                                    ...
                                    @if ($review->response)
                                        <div class="mt-4 bg-gray-50 p-3 rounded-md">
                                            <p class="text-sm font-medium text-gray-900">Response from
                                                {{ $artisan->name }}</p>
                                            <p class="text-sm text-gray-700 mt-1">{{ $review->response }}</p>
                                            @if ($review->response_date)
                                                <p class="text-xs text-gray-500 mt-1">
                                                    {{ $review->response_date->format('M d, Y') }}</p>
                                            @endif
                                        </div>
                                    @endif
                                </div>
...
@section('scripts')
    <script>
        ...
        function switchTab(tabId) {
            ...
            document.querySelector(`[onclick="switchTab('${tabId}')"]`).classList.add('active', 'text-amber-600',
                    ...
                    .then(data => {
                        ...
                        if (data.slots && data.slots.length > 0) {
                            ...
                            slotsContainer.innerHTML =
                                '<p class="text-gray-500 italic">No available time slots for this date</p>';
                        }
                    })
                    .catch(error => {
