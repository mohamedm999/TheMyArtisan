                    ...
                    @foreach ($artisans as $artisan)
                        <div
                            ...
                            <div class="relative">
                                ...
                                @auth
                                    <button
                                        class="save-artisan-btn absolute top-2 right-2 p-2 rounded-full {{ Auth::user()->favorites()->where('artisan_id', $artisan->id)->exists() ? 'bg-red-50' : 'bg-white' }} shadow-md hover:scale-105 transition-transform"
                                        data-artisan-id="{{ $artisan->id }}"
                                        data-is-saved="{{ Auth::user()->favorites()->where('artisan_id', $artisan->id)->exists() ? 'true' : 'false' }}"
                                    >
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="h-6 w-6 {{ Auth::user()->favorites()->where('artisan_id', $artisan->id)->exists() ? 'text-red-500' : 'text-gray-400' }}"
                                            fill="{{ Auth::user()->favorites()->where('artisan_id', $artisan->id)->exists() ? 'currentColor' : 'none' }}"
                                            viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div id="list-view" class="hidden">
                    <div class="bg-white shadow rounded-md overflow-hidden">
                        <ul class="divide-y divide-gray-200">
                            @foreach ($artisans as $artisan)
                                <li class="hover:bg-gray-50 relative">
                                    <a href="{{ route('client.artisans.show', $artisan->id) }}" class="block">
                                    ...
                                    @auth
                                        <button
                                            class="save-artisan-btn absolute top-4 right-4 p-2 rounded-full {{ Auth::user()->favorites()->where('artisan_id', $artisan->id)->exists() ? 'bg-red-50' : 'bg-white' }} shadow-sm hover:scale-105 transition-transform"
                                            data-artisan-id="{{ $artisan->id }}"
                                            data-is-saved="{{ Auth::user()->favorites()->where('artisan_id', $artisan->id)->exists() ? 'true' : 'false' }}"
                                        >
                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                class="h-5 w-5 {{ Auth::user()->favorites()->where('artisan_id', $artisan->id)->exists() ? 'text-red-500' : 'text-gray-400' }}"
                                                fill="{{ Auth::user()->favorites()->where('artisan_id', $artisan->id)->exists() ? 'currentColor' : 'none' }}"
                                                viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
...
@section('scripts')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Save artisan functionality
            document.querySelectorAll('.save-artisan-btn').forEach(button => {
