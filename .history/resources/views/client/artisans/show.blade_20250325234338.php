        ...
        <div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
            <div class="lg:flex lg:items-center lg:justify-between">
                ...
                <div class="mt-5 flex lg:mt-0 lg:ml-4 flex-col sm:flex-row gap-3">
                    <button id="save-artisan-btn"
                        class="sm:inline-flex sm:items-center px-5 py-2 border border-gray-300 text-base font-medium rounded-md {{ Auth::check() && Auth::user()->favorites()->where('artisan_id', $artisan->id)->exists() ? 'bg-red-50 text-red-700 border-red-300' : 'text-gray-700 bg-white hover:bg-gray-50' }} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
                        data-artisan-id="{{ $artisan->id }}"
                        data-is-saved="{{ Auth::check() && Auth::user()->favorites()->where('artisan_id', $artisan->id)->exists() ? 'true' : 'false' }}">
                        <svg xmlns="http://www.w3.org/2000/svg"
                            class="h-5 w-5 mr-2 {{ Auth::check() && Auth::user()->favorites()->where('artisan_id', $artisan->id)->exists() ? 'text-red-500' : 'text-gray-400' }}"
                            fill="{{ Auth::check() && Auth::user()->favorites()->where('artisan_id', $artisan->id)->exists() ? 'currentColor' : 'none' }}"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                        </svg>
                        <span>{{ Auth::check() && Auth::user()->favorites()->where('artisan_id', $artisan->id)->exists() ? 'Saved' : 'Save Artisan' }}</span>
                    </button>
                    <a href="#contact"
...
@section('scripts')
    <script>
        ...
        // Add save artisan functionality
        document.addEventListener('DOMContentLoaded', function() {
            const saveButton = document.getElementById('save-artisan-btn');
            ...
            if (saveButton) {
                saveButton.addEventListener('click', function(e) {
                    ...
                    // The endpoint for saving/unsaving
                    const url = isSaved
                        ? `/api/unsave-artisan/${artisanId}`
                        : `/api/save-artisan/${artisanId}`;
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            button.dataset.isSaved = newSavedState.toString();
                            ...
                            if (newSavedState) {
                                ...
                                // Show notification
                                showNotification('Artisan added to favorites');
                            } else {
                                button.classList.remove('bg-red-50', 'text-red-700', 'border-red-300');
                                button.classList.add('text-gray-700', 'bg-white', 'hover:bg-gray-50');
                                icon.classList.remove('text-red-500');
                                icon.classList.add('text-gray-400');
                                icon.setAttribute('fill', 'none');
                                text.textContent = 'Save Artisan';

                                // Show notification
                                showNotification('Artisan removed from favorites');
                            }
                        } else {
                            showNotification(data.message || 'Could not update favorites', 'error');
                        }
                    })
                    .catch(error => {
            ...
            function showNotification(message, type = 'success') {
                ...
                } text-white text-sm z-50 transform transition-all duration-500 translate-y-20 opacity-0`;
