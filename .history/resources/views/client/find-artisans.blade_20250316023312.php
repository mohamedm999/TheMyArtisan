            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white">
                        ...
                        <form action="{{ route('client.find-artisans') }}" method="GET" class="space-y-4">
                            ...
                            <div class="flex justify-end">
                                @if(request()->anyFilled(['search', 'category', 'location', 'sort']))
                                    <a href="{{ route('client.find-artisans') }}" class="mr-2 px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded-md text-gray-700 text-sm font-medium">
                                        Clear Filters
                                    </a>
                                @endif
                                <button type="submit" class="px-4 py-2 bg-amber-600 hover:bg-amber-700 rounded-md text-white text-sm font-medium">
                                    Apply Filters
                                </button>
                            </div>
                        </form>
                    ...
                    <div class="mb-4 text-gray-600">
                        Showing {{ $artisans->firstItem() ?? 0 }} to {{ $artisans->lastItem() ?? 0 }} of {{ $artisans->total() ?? 0 }} artisans
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @if (isset($artisans) && $artisans->count() > 0)
                            @foreach ($artisans as $artisan)
                                <div class="bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow border border-gray-200">
                                    <div class="relative">
                                    ...
                                    <div class="p-4 pt-10">
                                        <div class="flex items-center justify-between">
                                            <div>
                                                <h3 class="font-medium text-lg">{{ $artisan->user->firstname ?? '' }} {{ $artisan->user->lastname ?? 'Unknown Artisan' }}</h3>
                                        ...
                                        <div class="mt-3">
                                            ...
                                            @if($artisan->hourly_rate)
                                                <div class="text-sm text-gray-600 mb-2">
                                                    <span class="font-semibold">€{{ $artisan->hourly_rate }}</span> per hour
                                                </div>
                                            @endif
                        ...
                        @else
                            <div class="col-span-3 text-center py-12">
                                ...
                                @if(request()->anyFilled(['search', 'category', 'location']))
                                    <div class="mt-6">
                                        <a href="{{ route('client.find-artisans') }}" class="text-amber-600 hover:text-amber-500">
                                            Clear all filters
                                        </a>
                                    </div>
                                @endif
