    ...
    <div class="px-4 py-6 sm:px-0">
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            ...
            <div class="px-4 py-5 sm:p-6">
                <form action="{{ route('client.reviews.store', $booking->id) }}" method="POST">
                    @csrf

                    <div class="mb-6">
                        <h2 class="text-lg font-medium text-gray-900">{{ $booking->service->name }}</h2>
                        <p class="text-sm text-gray-600">Completed on {{ \Carbon\Carbon::parse($booking->completed_at)->format('M d, Y') }}</p>
                    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        ratingLabels.forEach((label, index) => {
            label.addEventListener('mouseover', function() {
                // Reset all stars
                ratingLabels.forEach(l => l.classList.remove('text-yellow-400', 'text-gray-300'));
