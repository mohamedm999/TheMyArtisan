                        ...
                        <div class="w-full md:w-1/3 border-r border-gray-200 overflow-y-auto">
                            ...
                            @else
                                ...
                                @foreach ($conversations as $conv)
                                    ...
                                    @endphp
                                    <a href="{{ route('client.message.show', $conv->id) }}"
                                        class="block px-4 py-3 border-b border-gray-200 hover:bg-gray-50 {{ $isActive ? 'bg-green-50' : '' }}">
                                        <div class="flex items-center">
                                            <div
                                                class="h-10 w-10 rounded-full bg-green-100 flex items-center justify-center text-green-600 font-bold mr-3 relative">
                                                {{ $initials }}
                                                @if ($unreadCount > 0)
                                                    <span
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <div class="flex justify-between items-baseline">
                                                    <h3 class="font-medium text-gray-800 truncate">
                                                        {{ $otherUser->firstname }} {{ $otherUser->lastname }}</h3>
                                                    <span class="text-xs text-gray-500">{{ $timeAgo }}</span>
                                                </div>
                                                @if ($latestMessage)
                                                    <p
                                                        class="text-sm {{ $unreadCount > 0 ? 'font-medium text-gray-800' : 'text-gray-500' }} truncate">
                        ...
                        <div class="flex-1 flex flex-col">
                            @if (!isset($conversation))
                                ...
                                <div class="flex-1 flex flex-col items-center justify-center p-4 text-center">
                                    ...
                                    <p class="text-gray-500 max-w-sm mb-4">Select a conversation or start a new one with an
                                        artisan</p>
                            ...
                            @else
                                ...
                                <div class="flex-1 flex flex-col">
                                    ...
                                    <div class="p-4 border-b border-gray-200 flex items-center">
                                        ...
                                        <div>
                                            <h3 class="font-medium text-gray-800">{{ $otherUser->firstname }}
                                                {{ $otherUser->lastname }}</h3>
                                            <p class="text-xs text-gray-500">{{ $serviceType }}</p>
                                        </div>
                                        <button
                                            class="ml-auto p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-full">
                                    ...
                                    <div class="p-4 border-t border-gray-200">
                                        <form action="{{ route('messages.send', $conversation->id) }}" method="POST"
                                            ...
                                            <button type="submit"
                                                class="ml-2 p-2 bg-green-600 text-white rounded-full hover:bg-green-700">
