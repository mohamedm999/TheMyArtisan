        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <button id="addAvailabilityBtn" class="px-4 py-2 bg-amber-600 text-white rounded hover:bg-amber-700 focus:outline-none">
                ...
                <div class="flex mb-6 space-x-4">
                    <div class="w-1/2">
                        ...
                        <select id="month" class="w-full rounded-md border-gray-300 shadow-sm focus:border-amber-300 focus:ring focus:ring-amber-200 focus:ring-opacity-50">
                    ...
                    <div class="w-1/2">
                        ...
                        <select id="year" class="w-full rounded-md border-gray-300 shadow-sm focus:border-amber-300 focus:ring focus:ring-amber-200 focus:ring-opacity-50">
                ...
                <div class="bg-white rounded-lg shadow overflow-hidden">
                    ...
                    <div class="grid grid-cols-7 gap-px bg-gray-200">
                        @foreach ($calendarDays as $day)
                            <div
                                class="min-h-[100px] bg-white p-2 {{ $day['today'] ? 'bg-amber-50' : '' }} {{ !$day['current_month'] ? 'bg-gray-50 text-gray-400' : '' }}"
                                data-date="{{ $day['date'] }}"
                            >
                                <div class="flex justify-between mb-1">
                                    <span class="text-sm {{ $day['today'] ? 'font-bold text-amber-600' : '' }}">{{ $day['day'] }}</span>
                                ...
                                <div class="space-y-1">
                                    @if(isset($day['availabilities']) && count($day['availabilities']) > 0)
                                        @foreach($day['availabilities'] as $availability)
                                            <div class="text-xs p-1 rounded {{ $availability['status'] == 'available' ? 'bg-green-100 text-green-800' : ($availability['status'] == 'booked' ? 'bg-blue-100 text-blue-800' : 'bg-red-100 text-red-800') }}">
                                                {{ $availability['start_time'] }} - {{ $availability['end_time'] }}
                                                @if($availability['status'] == 'available')
                                                    <button class="float-right text-green-600 hover:text-green-800 delete-availability" data-id="{{ $availability['id'] }}">
...
<div id="availabilityModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full" aria-modal="true">
        ...
        <div class="mt-3">
            ...
            <form id="availabilityForm" action="{{ route('artisan.schedule.store') }}" method="POST">
                ...
                <div class="mb-4" id="repeatUntilField" style="display: none;">
                    <label for="repeat_until" class="block text-sm font-medium text-gray-700 mb-1">Repeat Until</label>
