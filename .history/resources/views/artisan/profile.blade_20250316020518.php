                            ...
                            <div class="bg-amber-50 p-6 rounded-lg shadow">
                                <div class="flex flex-col items-center">
                                    <div class="w-32 h-32 rounded-full overflow-hidden mb-4">
                                        @if (Auth::user()->profile_image || isset($artisan->image))
                                            <img src="{{ asset('storage/' . (Auth::user()->profile_image ?? ($artisan->image ?? ''))) }}"
                                ...
                                <div class="mt-6">
                                    ...
                                    <div class="space-y-2 text-sm">
                                        ...
                                        <p class="flex items-center">
                                            ...
                                            {{ isset($artisanProfile) && $artisanProfile ? $artisanProfile->full_address ?? 'Paris, France' : 'Paris, France' }}
                                        </p>
                                    </div>
                                    ...
                                    @if (isset($certifications) && count($certifications) > 0)
                                        @foreach ($certifications as $certification)
                                            <div class="bg-gray-50 p-4 rounded-lg relative">
                                                <h4 class="font-medium">{{ $certification->title }}</h4>
                                                <p class="text-sm text-gray-500">Issued by:
                                                    {{ $certification->issuing_organization }}</p>
                                                <p class="text-sm text-gray-500">Valid until:
                                                    @if (isset($certification->expiry_date) && !empty($certification->expiry_date))
    ...
    @if (config('app.debug'))
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mt-6">
            <div class="bg-red-50 p-6 rounded-lg shadow border border-red-200">
                <h3 class="text-lg font-medium text-red-800 mb-4">Debug Information</h3>

                <div class="space-y-4">
                    <div>
                        <h4 class="text-md font-medium text-red-700">Artisan Profile</h4>
                        @if (isset($artisanProfile) && $artisanProfile)
                            <pre class="bg-white p-2 rounded text-xs overflow-auto">ID: {{ $artisanProfile->id }}, User ID: {{ $artisanProfile->user_id }}</pre>
                        @else
                            <p class="text-sm text-red-600">No artisan profile found</p>
                        @endif
                    </div>

                    <div>
                        <h4 class="text-md font-medium text-red-700">Work Experiences</h4>
                        <p class="text-sm">Count: {{ isset($workExperiences) ? $workExperiences->count() : 0 }}</p>
                        @if (isset($workExperiences) && $workExperiences->count() > 0)
                            <pre class="bg-white p-2 rounded text-xs overflow-auto">{{ json_encode($workExperiences->toArray(), JSON_PRETTY_PRINT) }}</pre>
                        @else
                            <p class="text-sm text-red-600">No work experiences found</p>
                        @endif
                    </div>

                    <div>
                        <h4 class="text-md font-medium text-red-700">Certifications</h4>
                        <p class="text-sm">Count: {{ isset($certifications) ? $certifications->count() : 0 }}</p>
                        @if (isset($certifications) && $certifications->count() > 0)
                            <pre class="bg-white p-2 rounded text-xs overflow-auto">{{ json_encode($certifications->toArray(), JSON_PRETTY_PRINT) }}</pre>
                        @else
                            <p class="text-sm text-red-600">No certifications found</p>
                        @endif
                    </div>

                    @if (isset($debugInfo))
                        <div>
                            <h4 class="text-md font-medium text-red-700">Additional Debug Info</h4>
                            <pre class="bg-white p-2 rounded text-xs overflow-auto">{{ json_encode($debugInfo, JSON_PRETTY_PRINT) }}</pre>
                        </div>
                    @endif
                </div>

                <div class="mt-4">
                    <h4 class="text-md font-medium text-red-700">Verify Models</h4>
                    <div class="grid grid-cols-2 gap-4 mt-2">
                        <div>
                            <h5 class="text-sm font-medium">Check WorkExperience Model</h5>
                            @php
                                $workExpClass = class_exists('App\\Models\\WorkExperience');
                                if ($workExpClass) {
                                    $workExpModel = new App\Models\WorkExperience();
                                    $workExpTable = $workExpModel->getTable();
                                    $workExpFillable = $workExpModel->getFillable();
                                }
                            @endphp
                            <pre class="bg-white p-2 rounded text-xs overflow-auto">
Class exists: {{ $workExpClass ? 'Yes' : 'No' }}
@if ($workExpClass)
Table: {{ $workExpTable }}
Fillable: {{ json_encode($workExpFillable) }}
@endif
                            </pre>
                        </div>
                        <div>
                            <h5 class="text-sm font-medium">Check Certification Model</h5>
                            @php
                                $certClass = class_exists('App\\Models\\Certification');
                                if ($certClass) {
                                    $certModel = new App\Models\Certification();
                                    $certTable = $certModel->getTable();
                                    $certFillable = $certModel->getFillable();
                                }
                            @endphp
                            <pre class="bg-white p-2 rounded text-xs overflow-auto">
Class exists: {{ $certClass ? 'Yes' : 'No' }}
@if ($certClass)
Table: {{ $certTable }}
Fillable: {{ json_encode($certFillable) }}
@endif
                            </pre>
                        </div>
                    </div>
