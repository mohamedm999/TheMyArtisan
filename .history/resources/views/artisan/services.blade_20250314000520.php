                ...
                <div class="p-6 bg-white border-b border-gray-200">
                            ...
                            @foreach($services as $service)
                                <div class="bg-white rounded-lg border border-gray-200 shadow-md overflow-hidden">
                                    <div class="h-48 bg-gray-200 relative">
                                        @if($service->image)
                                            <img class="w-full h-full object-cover" src="{{ Storage::url($service->image) }}" alt="{{ $service->name }}">
                                        @else
                                        ...
                                        <div class="absolute top-2 right-2 flex space-x-1">
                                            <button type="button" onclick="toggleModal('editServiceModal{{ $service->id }}')"
                                                class="bg-amber-100 text-amber-600 p-2 rounded-full hover:bg-amber-200">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <form action="{{ route('artisan.services.delete', $service->id) }}" method="POST"
                                                onsubmit="return confirm('Are you sure you want to delete this service?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="bg-red-100 text-red-600 p-2 rounded-full hover:bg-red-200">
                                    ...
                                    <div class="p-5">
                                        <div class="flex justify-between items-start mb-2">
                                            ...
                                            <span class="bg-amber-100 text-amber-800 text-sm font-medium px-2.5 py-0.5 rounded">
                                        ...
                                        <div class="flex items-center justify-between">
                                            ...
                                            <span class="text-sm text-gray-500">
                                                ...
                                                @else
                                                    <span class="text-gray-400"><i class="fas fa-circle text-xs mr-1"></i> Inactive</span>
                                                @endif
                                            </span>
                                        </div>
                                    </div>
                                ...
                                <!-- Edit Service Modal -->
                                <div id="editServiceModal{{ $service->id }}" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
                                    <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white">
                                        <div class="mt-3">
                                            <h3 class="text-lg font-medium text-gray-900 mb-4 text-center">Edit Service</h3>
                                            <form action="{{ route('artisan.services.update', $service->id) }}" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                @method('PUT')
                                                <!-- Form fields -->
                                                <div class="mb-4">
                                                    <label class="block text-gray-700 text-sm font-bold mb-2" for="edit_name{{ $service->id }}">
                                                        Service Name
                                                    </label>
                                                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                                        id="edit_name{{ $service->id }}" name="name" type="text" value="{{ $service->name }}" required>
                                                </div>
                                                <div class="mb-4">
                                                    <label class="block text-gray-700 text-sm font-bold mb-2" for="edit_description{{ $service->id }}">
                                                        Description
                                                    </label>
                                                ...
                                                <div class="grid grid-cols-2 gap-4 mb-4">
                                                    <div>
                                                        <label class="block text-gray-700 text-sm font-bold mb-2" for="edit_price{{ $service->id }}">
                                                            Price (€)
                                                        </label>
                                                        <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                                            id="edit_price{{ $service->id }}" name="price" type="number" step="0.01" min="0" value="{{ $service->price }}" required>
                                                    </div>
                                                    <div>
                                                        <label class="block text-gray-700 text-sm font-bold mb-2" for="edit_duration{{ $service->id }}">
                                                            Duration (minutes)
                                                        </label>
                                                        <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                                            id="edit_duration{{ $service->id }}" name="duration" type="number" min="1" value="{{ $service->duration }}" required>
                                                    </div>
                                                </div>
                                                <div class="mb-4">
                                                    <label class="block text-gray-700 text-sm font-bold mb-2" for="edit_image{{ $service->id }}">
                                                        Service Image
                                                    </label>
                                                    <input type="file" id="edit_image{{ $service->id }}" name="image" class="w-full text-sm text-gray-500
                                                        file:mr-4 file:py-2 file:px-4
                                                        file:rounded file:border-0
                                                        file:text-sm file:font-semibold
                                                        file:bg-amber-50 file:text-amber-700
                                                        hover:file:bg-amber-100">
                                                    <p class="text-xs text-gray-500 mt-1">Current image: {{ $service->image ? basename($service->image) : 'No image' }}</p>
                                                </div>
                                                <div class="mb-4">
                                                    <label class="flex items-center">
                                                        <input type="checkbox" name="is_active" class="form-checkbox h-4 w-4 text-amber-600" {{ $service->is_active ? 'checked' : '' }}>
                    ...
                    @else
                        <div class="bg-gray-50 p-12 rounded-lg text-center">
                            ...
                            <p class="text-gray-500 mb-6">You haven't added any services yet. Add your first service to start attracting clients.</p>
                            <button type="button" onclick="toggleModal('addServiceModal')"
            ...
            <div class="mt-3">
                ...
                <form action="{{ route('artisan.services.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                            id="name" name="name" type="text" placeholder="e.g. Furniture Restoration" required>
                    </div>
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="file" id="image" name="image" class="w-full text-sm text-gray-500
                            file:mr-4 file:py-2 file:px-4
                            file:rounded file:border-0
                            file:text-sm file:font-semibold
                            file:bg-amber-50 file:text-amber-700
