                            ...
                            <div class="divide-y divide-gray-200">
                                @if (count($conversations ?? []) === 0)
                                    ...
                                    <div class="flex flex-col items-center justify-center h-64 p-4 text-center">
                                        ...
                                        <p class="text-sm text-gray-500">Clients will contact you here when interested in
                                            your services</p>
                                    </div>
                                @else
                                    <!-- Conversations list -->
                                    @foreach ($conversations as $conv)
                                        @php
                                            ...
                                            $timeAgo = $latestMessage
                                                ...
                                                : '';
                                            $initials = strtoupper(
                                                substr($otherUser->firstname, 0, 1) .
                                                    substr($otherUser->lastname, 0, 1),
                                            );
                                        @endphp
                                        <a href="{{ route('artisan.message.show', $conv->id) }}" class="block">
                                            <div
                                                class="p-4 flex items-start {{ $isActive ? 'bg-amber-50 border-l-4 border-amber-500' : 'hover:bg-gray-50 cursor-pointer' }}">
                                                <div class="flex-shrink-0 relative">
                                                    ...
                                                    @if ($unreadCount > 0)
                                                        <span
                                                            class="absolute -top-1 -right-1 h-4 w-4 rounded-full bg-red-500"></span>
                                                    @endif
                                                </div>
                                                <div class="ml-3 flex-1">
                                                    <div class="flex justify-between items-baseline">
                                                        <h3 class="text-sm font-medium">{{ $otherUser->firstname }}
                                                            {{ $otherUser->lastname }}</h3>
                                                        <span class="text-xs text-gray-500">{{ $timeAgo }}</span>
                                                    </div>
                                                    @if ($latestMessage)
                                                        <p
                                                            class="text-sm {{ $unreadCount > 0 ? 'font-medium' : 'text-gray-600' }} truncate">
                        ...
                        <div class="w-full lg:w-2/3 flex flex-col h-full">
                            @if (!isset($conversation))
                                ...
                                <div class="flex-1 flex flex-col items-center justify-center p-4 text-center">
                                    ...
                                    <p class="text-gray-500 max-w-sm mb-4">Select a conversation to view messages from your
                                        clients</p>
                                </div>
                            @else
                                <!-- Conversation Header -->
                                @php
                                    ...
                                    $initials = strtoupper(
                                        substr($otherUser->firstname, 0, 1) . substr($otherUser->lastname, 0, 1),
                                    );
                                @endphp
                                <div class="p-4 border-b border-gray-200 flex items-center">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-3">
                                            <h3 class="text-sm font-medium">{{ $otherUser->firstname }}
                                                {{ $otherUser->lastname }}</h3>
                                            <p class="text-xs text-gray-500">Last active:
                                                {{ $otherUser->updated_at->diffForHumans() }}</p>
                                        </div>
                                    </div>
                                    <div class="ml-auto">
                                    ...
                                    @foreach ($conversation->messages as $message)
                                        @php
                                            ...
                                            $senderInitials = strtoupper(
                                                substr($message->sender->firstname, 0, 1) .
                                                    substr($message->sender->lastname, 0, 1),
                                            );
                                        @endphp
                                        ...
                                        @if ($showDate)
                                            <div class="flex justify-center">
                                                <span
                                                    class="bg-gray-100 text-gray-500 text-xs px-2 py-1 rounded">{{ $message->created_at->format('F j, Y') }}</span>
                                            </div>
                                        @endif
                                        ...
                                        @if ($isMine)
                                            ...
                                            <div class="flex items-end justify-end my-4">
                                                <span
                                                    class="text-xs text-gray-500 mr-2">{{ $message->created_at->format('g:i A') }}</span>
                                            ...
                                            <div class="flex items-end my-4">
                                                <div class="flex-shrink-0 mr-2">
                                                    <div
                                                        ...
                                                        <span
                                                            class="text-gray-700 font-medium text-xs">{{ $senderInitials }}</span>
                                                    </div>
                                                </div>
                                                ...
                                                <span
                                                    class="text-xs text-gray-500 ml-2">{{ $message->created_at->format('g:i A') }}</span>
                                            </div>
                                        @endif
                                    @endforeach
                                </div>
                                ...
                                <div class="p-4 border-t border-gray-200">
                                    <form action="{{ route('messages.send', $conversation->id) }}" method="POST"
                                        ...
                                        <div class="flex space-x-2">
                                            <button type="button"
                                                class="p-2 rounded-full bg-gray-100 text-gray-500 hover:bg-gray-200">
                                                <i class="fas fa-paperclip"></i>
                                            </button>
                                            <button type="submit"
                                                class="p-2 rounded-full bg-amber-600 text-white hover:bg-amber-700">
