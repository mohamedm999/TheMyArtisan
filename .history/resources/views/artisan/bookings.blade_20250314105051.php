                                ...
                                <div class="flex flex-col md:flex-row">
                                    <div class="md:w-1/4 bg-gray-50 p-4 flex flex-col justify-between">
                                        <div>
                                            ...
                                            <p class="text-sm text-gray-500 mt-1">Service ID: #{{ $booking->service->service_id }}</p>
                                        </div>
                                        <div class="mt-4 md:mt-0">
                                            <p class="text-sm font-medium text-gray-500">Price</p>
                                            <p class="text-lg font-semibold text-amber-600">€{{ number_format($booking->price, 2) }}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="md:w-2/4 p-4 border-t md:border-t-0 md:border-l md:border-r border-gray-200">
                                        <div class="flex items-center mb-3">
                                            <div
                                                ...
                                                @if($booking->customer->avatar)
                                                    <img src="{{ asset('storage/' . $booking->customer->avatar) }}" alt="{{ $booking->customer->name }}" class="h-10 w-10 rounded-full">
                                                @else
                                                    <span class="text-gray-500 font-medium">{{ substr($booking->customer->name, 0, 1) }}{{ substr(explode(' ', $booking->customer->name)[1] ?? '', 0, 1) }}</span>
                                                @endif
                                            </div>
                                            <div class="ml-4">
                                                <h4 class="text-sm font-medium text-gray-900">{{ $booking->customer->name }}</h4>
                                                <p class="text-sm text-gray-500">{{ $booking->customer->city }}, {{ $booking->customer->country }}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="space-y-2">
                                            <div class="flex items-center text-sm">
                                                ...
                                                <span>Requested for: <strong>{{ $booking->requested_date->format('F d, Y') }}</strong></span>
                                            </div>
                                            <div class="flex items-center text-sm">
                                                <i class="fas fa-clock text-amber-500 mr-2 w-4"></i>
                                                <span>Estimated completion: <strong>{{ $booking->estimated_completion }}</strong></span>
                                            </div>
                                    ...
                                    <div
                                        ...
                                        <div class="text-sm text-gray-500">
                                            <p>Requested: <span class="text-gray-900">{{ $booking->created_at->format('M d, Y') }}</span></p>
                                        </div>
