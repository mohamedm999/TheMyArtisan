...
@section('content')
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="lg:col-span-1">
                            <div class="bg-amber-50 p-6 rounded-lg shadow">
                                ...
                                <div class="mt-6">
                                    <h3 class="font-medium text-gray-700 mb-2">Specialty Categories</h3>
                                    <div class="flex flex-wrap gap-2 mt-2">
                                        @if (isset($artisanProfile) && isset($selectedCategories) && count($selectedCategories) > 0)
                                            @foreach ($artisanProfile->categories as $category)
                                                <span
                                            ...
                                            @endforeach
                                        @else
                                            <p class="text-sm text-gray-500">No specialty categories selected.</p>
                                        @endif
                                    </div>

                                    <button type="button" onclick="toggleModal('categoriesModal')"
                                        class="mt-4 w-full inline-flex justify-center items-center px-3 py-1 bg-amber-100 text-amber-600 border border-transparent rounded-md font-medium text-xs uppercase tracking-widest hover:bg-amber-200 active:bg-amber-300 focus:outline-none focus:ring ring-amber-200 transition ease-in-out duration-150">
                                        <i class="fas fa-tags mr-1"></i> Select Categories
                                    </button>
                                </div>
                            </div>
                        </div>
                ...
                <form class="mt-4" action="{{ route('artisan.profile.contact-info') }}" method="POST">
                        ...
                        <select name="country_id" id="country_id"
                            ...
                            @foreach($countries as $country)
                                <option value="{{ $country->id }}"
                                    {{ isset($artisanProfile) && $artisanProfile && $artisanProfile->country_id == $country->id ? 'selected' : '' }}>
                                    {{ $country->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="city_id" id="city_id"
                            ...
                            @foreach($cities as $city)
                                <option value="{{ $city->id }}"
                                    {{ isset($artisanProfile) && $artisanProfile && $artisanProfile->city_id == $city->id ? 'selected' : '' }}>
                                    {{ $city->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4 text-left">
                        ...
                        <input type="text" name="state" id="state"
                            value="{{ isset($artisanProfile) && $artisanProfile ? $artisanProfile->state : '' }}"
                            class="mt-1 focus:ring-amber-500 focus:border-amber-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    </div>
                    <div class="mb-4 text-left">
    ...
    <div id="categoriesModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
        <div class="relative top-10 mx-auto p-5 border w-11/12 max-w-4xl shadow-lg rounded-md bg-white">
            <div class="mt-3">
                <h3 class="text-lg leading-6 font-medium text-gray-900 text-center mb-4">Select Your Specialty Categories
                </h3>

                <form action="{{ route('artisan.profile.categories') }}" method="POST" id="categoriesForm">
                    @csrf
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-h-96 overflow-y-auto p-2">
                        @foreach ($categories as $category)
                            <div class="category-card border rounded-lg overflow-hidden cursor-pointer
                                      {{ in_array($category->id, $selectedCategories ?? []) ? 'ring-2 ring-amber-500' : '' }}"
                                ...
                                <div class="h-32 bg-gray-200 overflow-hidden">
                                    ...
                                    @else
                                        <div
                                            class="w-full h-full flex items-center justify-center bg-amber-100 text-amber-800">
                                            <i class="fas fa-tools text-3xl"></i>
                                        </div>
                                    @endif
                                </div>
                                <div class="p-3 bg-white">
                                    <h4 class="font-medium text-gray-900 text-center">{{ $category->name }}</h4>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="flex justify-between mt-6">
                        <button type="button" onclick="toggleModal('categoriesModal')"
                            class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md">Cancel</button>
                        <button type="submit" class="px-4 py-2 bg-amber-600 text-white rounded-md">Save
                            Categories</button>
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Add click event handlers for category cards
            ...
            if (countrySelect && citySelect) {
                countrySelect.addEventListener('change', function() {
                    ...
                    if (countryId) {
                        ...
                        fetch(`/artisan/get-cities?country_id=${countryId}`)
