    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <div class="bg-amber-50 rounded-lg p-4 mb-6">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="text-center">
                            <div class="text-4xl font-bold text-amber-600">{{ number_format($stats['average_rating'], 1) }}</div>
                        ...
                        <div class="flex flex-col justify-center">
                            <div class="flex items-center mb-1">
                                ...
                                <div class="flex-grow bg-gray-200 rounded-full h-2.5 mr-2">
                                    <div class="bg-amber-500 h-2.5 rounded-full" style="width: {{ $stats['total'] > 0 ? ($stats['five_stars'] / $stats['total'] * 100) : 0 }}%"></div>
                                </div>
                            ...
                            <div class="flex items-center mb-1">
                                ...
                                <div class="flex-grow bg-gray-200 rounded-full h-2.5 mr-2">
                                    <div class="bg-amber-500 h-2.5 rounded-full" style="width: {{ $stats['total'] > 0 ? ($stats['four_stars'] / $stats['total'] * 100) : 0 }}%"></div>
                                </div>
                            ...
                            <div class="flex items-center mb-1">
                                ...
                                <div class="flex-grow bg-gray-200 rounded-full h-2.5 mr-2">
                                    <div class="bg-amber-500 h-2.5 rounded-full" style="width: {{ $stats['total'] > 0 ? ($stats['three_stars'] / $stats['total'] * 100) : 0 }}%"></div>
                                </div>
                            ...
                            <div class="flex items-center mb-1">
                                ...
                                <div class="flex-grow bg-gray-200 rounded-full h-2.5 mr-2">
                                    <div class="bg-amber-500 h-2.5 rounded-full" style="width: {{ $stats['total'] > 0 ? ($stats['two_stars'] / $stats['total'] * 100) : 0 }}%"></div>
                                </div>
                            ...
                            <div class="flex items-center">
                                ...
                                <div class="flex-grow bg-gray-200 rounded-full h-2.5 mr-2">
                                    <div class="bg-amber-500 h-2.5 rounded-full" style="width: {{ $stats['total'] > 0 ? ($stats['one_star'] / $stats['total'] * 100) : 0 }}%"></div>
                                </div>
                ...
                <!-- Search & Filter -->
                <div class="mb-6">
                    <form action="{{ route('artisan.reviews') }}" method="GET" class="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-4">
                        <div class="flex-grow">
                        ...
                        <div>
                            <select name="filter" id="filter"
                                ...
                                <option value="positive" {{ $filter == 'positive' ? 'selected' : '' }}>Positive (4-5 stars)</option>
                                <option value="neutral" {{ $filter == 'neutral' ? 'selected' : '' }}>Neutral (3 stars)</option>
                                <option value="negative" {{ $filter == 'negative' ? 'selected' : '' }}>Negative (1-2 stars)</option>
                                <option value="pending" {{ $filter == 'pending' ? 'selected' : '' }}>Needs Response</option>
                            </select>
                        </div>
                        <div>
                ...
                <div class="space-y-6">
                    @forelse ($reviews as $review)
                        <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
                            <div class="p-4 border-b border-gray-200">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center">
                                            @if ($review->customer->avatar)
                                                <img src="{{ asset('storage/' . $review->customer->avatar) }}" alt="{{ $review->customer->name }}" class="h-10 w-10 rounded-full">
                                            @else
                                                <span class="text-gray-500 font-medium">{{ substr($review->customer->firstname, 0, 1) }}{{ substr($review->customer->lastname, 0, 1) }}</span>
                                            @endif
                                        </div>
                                        <div class="ml-4">
                                            <h4 class="text-sm font-medium text-gray-900">{{ $review->customer->firstname }} {{ $review->customer->lastname }}</h4>
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <span class="text-sm font-semibold">{{ number_format($review->rating, 1) }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="p-4">
                                ...
                                @if ($review->response)
                                    <div class="mb-3 bg-gray-50 p-3 rounded-md">
                                        ...
                                        <p class="text-xs text-gray-500 mt-1">Responded on {{ $review->response_date->format('F d, Y') }}</p>
                                    </div>
                                @else
                                    <div class="mb-3">
                                        <button class="text-sm text-amber-600 hover:text-amber-800 respond-btn" data-review-id="{{ $review->id }}">
                            ...
                            <div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
                                @if (!$review->reported)
                                    <button class="text-xs text-gray-500 hover:text-gray-700 report-btn" data-review-id="{{ $review->id }}">
                                        <i class="fas fa-flag mr-1"></i> Report this review
                                    </button>
                                @else
                                    <span class="text-xs text-gray-500">
                                        <i class="fas fa-flag mr-1"></i> Reported on {{ $review->report_date->format('F d, Y') }}
                                    </span>
                                @endif
                            </div>
                        </div>
                    @empty
                        <div class="text-center p-8">
                            ...
                            <p class="text-gray-500">When clients leave reviews for your services, they'll appear here.</p>
                        </div>
                    @endforelse
                </div>
...
<!-- Respond Modal -->
<div id="respondModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full" aria-modal="true">
            ...
            <form id="respondForm" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <textarea id="response" name="response" rows="4"
                        ...
                        placeholder="Write your response to the client's review..."
                        required></textarea>
                </div>
...
<!-- Report Modal -->
<div id="reportModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full" aria-modal="true">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
            ...
            <form id="reportForm" method="POST">
                @csrf
                <div class="mb-4">
                    <label for="reason" class="block text-sm font-medium text-gray-700 mb-1">Reason for reporting</label>
