                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        @foreach($services as $service)
                            <div class="service-item bg-amber-50 p-6 rounded-lg shadow">
                                @if($service->image)
                                    <img src="{{ asset('storage/' . $service->image) }}" alt="{{ $service->name }}" class="service-image w-full h-32 object-cover mb-4">
                                @else
                                    <div class="placeholder-image w-full h-32 bg-amber-200 flex items-center justify-center text-amber-600 text-3xl font-bold mb-4">No Image</div>
                                @endif
