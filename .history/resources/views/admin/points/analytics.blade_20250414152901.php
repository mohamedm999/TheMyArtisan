            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                ...
                <div class="bg-blue-50 rounded-lg p-4 border border-blue-100">
                    ...
                    <div class="mt-2 flex items-center">
                        ...
                        <span class="text-3xl font-bold text-blue-800">{{ number_format($totalPointsAwarded - $totalPoints) }}</span>
                    </div>
                </div>
            </div>
                            ...
                            @foreach($topClients as $index => $clientPoint)
                                <li class="py-3 flex items-center justify-between">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-8 w-8 flex items-center justify-center {{ $index < 3 ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800' }} rounded-full">
                                            <span class="font-bold text-sm">{{ $index + 1 }}</span>
                                        </div>
                                        <div class="ml-3">
                                            <p class="text-sm font-medium text-gray-900">{{ $clientPoint->user->firstname }} {{ $clientPoint->user->lastname }}</p>
                                            <p class="text-xs text-gray-500">{{ $clientPoint->user->email }}</p>
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        <span class="px-3 py-1 inline-flex text-sm font-semibold rounded-full bg-primary-100 text-primary-800">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Current Balance</th>
                                    <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Lifetime Points</th>
                                ...
                                @foreach($richestClients as $index => $clientPoint)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                            <span class="inline-flex items-center justify-center h-6 w-6 rounded-full {{ $index < 3 ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800' }}">
                                                {{ $index + 1 }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div class="h-8 w-8 rounded-full bg-primary-100 flex items-center justify-center text-primary-700">
                                                    <span class="font-medium">{{ substr($clientPoint->user->firstname, 0, 1) }}{{ substr($clientPoint->user->lastname, 0, 1) }}</span>
                                                </div>
                                                <div class="ml-3">
                                                    <div class="text-sm font-medium text-gray-900">{{ $clientPoint->user->firstname }} {{ $clientPoint->user->lastname }}</div>
                                                    <div class="text-xs text-gray-500">{{ $clientPoint->user->email }}</div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('admin.points.show', $clientPoint->user_id) }}" class="text-primary-600 hover:text-primary-900">
                                                Manage Points
                                            </a>
                                        </td>
                                    </tr>
