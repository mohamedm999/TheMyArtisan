                            ...
                            <div class="flex flex-col items-center p-6 bg-gray-50 rounded-lg">
                                <div class="w-32 h-32 mb-4">
                                    @if($artisan->artisanProfile && $artisan->artisanProfile->profile_photo)
                                        <img src="{{ asset('storage/' . $artisan->artisanProfile->profile_photo) }}" alt="{{ $artisan->firstname }} {{ $artisan->lastname }}"
                                            class="w-full h-full rounded-full object-cover border-4 border-white shadow">
                                    @else
                                        <div class="w-full h-full rounded-full bg-gradient-to-r from-green-400 to-blue-500 flex items-center justify-center text-white text-4xl font-medium border-4 border-white shadow">
                                            {{ substr($artisan->firstname, 0, 1) }}{{ substr($artisan->lastname, 0, 1) }}
                                        </div>
                                    @endif
                                </div>

                                <h3 class="text-xl font-bold text-gray-800">{{ $artisan->firstname }} {{ $artisan->lastname }}</h3>
                                ...
                                <div class="mt-4 w-full">
                                    ...
                                    <div class="flex justify-between py-2 border-b">
                                        ...
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                            {{ $artisan->artisanProfile && $artisan->artisanProfile->status === 'approved' ? 'bg-green-100 text-green-800' : 
                                               ($artisan->artisanProfile && $artisan->artisanProfile->status === 'rejected' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800') }}">
                                ...
                                <div class="mb-6">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div class="font-medium">{{ $artisan->artisanProfile->phone ?? 'Not provided' }}</div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="mb-6">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <div class="font-medium">{{ $artisan->artisanProfile->specialty ?? 'Not specified' }}</div>
                                            </div>
                                            <div>
                                                <label class="block text-sm text-gray-500">Experience</label>
                                                <div class="font-medium">{{ $artisan->artisanProfile->years_experience ?? '0' }} years</div>
                                            </div>
                                            <div>
                                                <label class="block text-sm text-gray-500">Service Radius</label>
                                                <div class="font-medium">{{ $artisan->artisanProfile->service_radius ?? 'Not specified' }} km</div>
                                            </div>
                                            <div>
                                            ...
                                            @foreach($artisan->services as $service)
                                                <div class="border rounded-md p-3 bg-gray-50">
                                                    <div class="flex justify-between items-center mb-1">
                                                        ...
                                                        <span class="font-medium text-gray-800">{{ number_format($service->price, 2) }} DH</span>
                                                    </div>
                                                    ...
                                                    <div class="mt-2 flex items-center justify-between">
                                                        ...
                                                        <span class="text-xs text-gray-500">{{ $service->duration ?? 0 }} minutes</span>
                                                    </div>
                                                </div>
                                            @endforeach
                                        </div>
                                            ...
                                            <table class="min-w-full divide-y divide-gray-200">
                                                ...
                                                <tbody class="bg-white divide-y divide-gray-200">
                                                    @foreach($artisan->bookings->take(5) as $booking)
                                                        <tr>
                                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $booking->booking_date->format('M d, Y H:i') }}</td>
                                                            ...
                                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $booking->service->name ?? 'N/A' }}</td>
