    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="mb-6">
            <div class="flex justify-between items-center">
                ...
                <a href="{{ route('admin.artisans.index') }}" class="px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400">
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                        ...
                        <div class="bg-white rounded-lg shadow-sm p-6">
                            <div class="flex items-center mb-6">
                                <div class="flex-shrink-0 h-20 w-20 rounded-full overflow-hidden bg-gray-100 mr-4">
                                    ...
                                    @else
                                        <div class="h-20 w-20 bg-blue-100 text-blue-800 flex items-center justify-center rounded-full">
                                            <span class="text-2xl font-bold">{{ substr($artisan->firstname, 0, 1) }}{{ substr($artisan->lastname, 0, 1) }}</span>
                                        </div>
                                    @endif
                                </div>
                                <div>
                                    ...
                                    <p class="text-sm text-gray-500">Joined {{ $artisan->created_at->format('M d, Y') }}</p>
                                </div>
                            </div>
                            ...
                            <div class="border-t border-gray-200 pt-4">
                                ...
                                <ul class="space-y-2">
                                    ...
                                    <li class="flex items-start">
                                        ...
                                        <div class="ml-3 text-sm text-gray-700">{{ $artisan->artisanProfile->phone ?? 'Not provided' }}</div>
                                    </li>
                                    <li class="flex items-start">
                                        <div class="flex-shrink-0 h-5 w-5 text-gray-400">
                    ...
                    <div class="w-full lg:w-2/3 px-4">
                        <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                <div>
                                    ...
                                    <p class="text-gray-900">{{ ($artisan->artisanProfile && $artisan->artisanProfile->specialty) ? $artisan->artisanProfile->specialty : 'Not specified' }}</p>
                                </div>

                                <div>
                                    <h5 class="text-sm font-medium text-gray-700">Years of Experience</h5>
                                    <p class="text-gray-900">{{ ($artisan->artisanProfile && $artisan->artisanProfile->years_experience) ? $artisan->artisanProfile->years_experience : 'Not specified' }}</p>
                                </div>

                                <div>
                                    <h5 class="text-sm font-medium text-gray-700">Service Radius</h5>
                                    <p class="text-gray-900">{{ ($artisan->artisanProfile && $artisan->artisanProfile->service_radius) ? $artisan->artisanProfile->service_radius . ' km' : 'Not specified' }}</p>
                                </div>

                                <div>
                                    <h5 class="text-sm font-medium text-gray-700">Categories</h5>
                                    <div class="flex flex-wrap">
                                        @if($artisan->artisanProfile && isset($artisan->artisanProfile->categories) && $artisan->artisanProfile->categories->count() > 0)
                            ...
                            @if($artisan->artisanProfile)
                                ...
                                @if(isset($artisan->artisanProfile->skills) && is_array($artisan->artisanProfile->skills) && count($artisan->artisanProfile->skills) > 0)
                        ...
                        <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                            <div class="flex justify-between items-center mb-4">
                                <h4 class="text-sm font-medium text-gray-500 uppercase tracking-wider">Services ({{ $artisan->artisanProfile && isset($artisan->artisanProfile->services) ? $artisan->artisanProfile->services->count() : 0 }})</h4>
                                <a href="{{ route('admin.artisans.services', $artisan->id) }}" class="text-sm text-blue-600 hover:text-blue-900">View All</a>
                            </div>
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200">
                                            @foreach($artisan->artisanProfile->services->take(5) as $service)
                                                <tr>
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ number_format($service->price, 2) }} MAD</td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $service->duration }} minutes</td>
                        ...
                        <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                            <div class="flex justify-between items-center mb-4">
                                <h4 class="text-sm font-medium text-gray-500 uppercase tracking-wider">Recent Bookings ({{ $artisan->artisanProfile && isset($artisan->artisanProfile->bookings) ? $artisan->artisanProfile->bookings->count() : 0 }})</h4>
                            </div>

                            @if($artisan->artisanProfile && isset($artisan->artisanProfile->bookings) && $artisan->artisanProfile->bookings->count() > 0)
                                <div class="overflow-x-auto">
                                    <table class="min-w-full divide-y divide-gray-200">
                                        <thead class="bg-gray-50">
                                            <tr>
                        ...
                        <div class="bg-white rounded-lg shadow-sm p-6">
                            <div class="flex justify-between items-center mb-4">
                                <h4 class="text-sm font-medium text-gray-500 uppercase tracking-wider">Reviews ({{ $artisan->artisanProfile && isset($artisan->artisanProfile->reviews) ? $artisan->artisanProfile->reviews->count() : 0 }})</h4>
                            </div>
                ...
                <div class="mt-8 border-t border-gray-200 pt-6 flex flex-wrap justify-between">
                    <div>
                        <a href="{{ route('admin.artisans.services', $artisan->id) }}" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 mr-2">
                        ...
                        @if($artisan->artisanProfile)
                            ...
                            @if($status === 'pending')
                                <form method="POST" action="{{ route('admin.artisans.approve', $artisan->id) }}" class="inline">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 mr-2">
                                        <i class="fas fa-check mr-1"></i> Approve
                                    </button>
                                </form>

                                <form method="POST" action="{{ route('admin.artisans.reject', $artisan->id) }}" class="inline">
                                    @csrf
                                    @method('PUT')
