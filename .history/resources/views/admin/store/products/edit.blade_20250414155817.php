...
<div class="container px-6 mx-auto grid">
    ...
    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md">
        <form action="{{ route('admin.store.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                @if($product->image)
                    <div class="mb-3">
                        ...
                        <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="w-48 h-auto rounded-md border">
                    </div>
                @endif
                ...
                <input type="file" id="image" name="image" 
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" 
                    accept="image/*">
                <p class="mt-1 text-sm text-gray-500">Maximum file size: 2MB. Formats: JPG, PNG. Leave empty to keep current image.</p>
            </div>
            ...
            <div class="flex items-start mb-6">
                <div class="flex items-center h-5">
                    <input id="is_available" name="is_available" type="checkbox" value="1" {{ old('is_available', $product->is_available) ? 'checked' : '' }}
                        class="w-4 h-4 bg-gray-50 rounded border border-gray-300 focus:ring-3 focus:ring-blue-300">
                </div>
