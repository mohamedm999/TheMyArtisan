            ...
            <div class="w-full lg:w-1/3 px-2">
                <div class="bg-white rounded-lg shadow mb-4">
                    <div class="px-4 py-3 border-b">
                        <h6 class="font-bold text-blue-600">Statistics</h6>
                    </div>
                    <div class="p-4">
                        <div class="mb-4">
                            <div class="bg-white p-4 border-l-4 border-green-500 rounded shadow">
                                <div class="flex items-center">
                                    <div class="flex-grow">
                                        <div class="text-xs font-bold text-green-600 uppercase mb-1">Total Orders</div>
                                        <div class="text-xl font-bold text-gray-800">{{ $orderCount }}</div>
                                    </div>
                                    <div>
                        ...
                        <div class="mb-4">
                            <div class="bg-white p-4 border-l-4 border-blue-500 rounded shadow">
                                <div class="flex items-center">
                                    <div class="flex-grow">
                                        <div class="text-xs font-bold text-blue-600 uppercase mb-1">Points Spent</div>
                        ...
                        <h6 class="font-bold mb-2">Quick Actions</h6>
                        <div class="flex flex-wrap -mx-1">
                            <div class="w-1/2 px-1 mb-2">
                                <a href="{{ route('admin.store.products.edit', $product->id) }}"
                                    class="flex items-center justify-center px-2 py-1 text-sm bg-blue-600 text-white rounded hover:bg-blue-700 w-full transition">
                            ...
                            <div class="w-1/2 px-1 mb-2">
                                <form action="{{ route('admin.store.products.toggleAvailability', $product->id) }}" method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="flex items-center justify-center px-2 py-1 text-sm {{ $product->is_available ? 'bg-yellow-500' : 'bg-green-600' }} text-white rounded hover:{{ $product->is_available ? 'bg-yellow-600' : 'bg-green-700' }} w-full transition">
                                        @if ($product->is_available)
                            ...
                            <div class="w-1/2 px-1 mb-2">
                                <form action="{{ route('admin.store.products.toggleFeatured', $product->id) }}" method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="flex items-center justify-center px-2 py-1 text-sm {{ $product->is_featured ? 'bg-gray-500' : 'bg-yellow-500' }} text-white rounded hover:{{ $product->is_featured ? 'bg-gray-600' : 'bg-yellow-600' }} w-full transition">
                                        @if ($product->is_featured)
                            ...
                            <div class="w-1/2 px-1 mb-2">
                                <form action="{{ route('admin.store.products.destroy', $product->id) }}" method="POST"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="flex items-center justify-center px-2 py-1 text-sm bg-red-600 text-white rounded hover:bg-red-700 w-full transition">
                ...
                @if ($recentOrders->count() > 0)
                    <div class="bg-white rounded-lg shadow mb-4">
                        <div class="px-4 py-3 border-b">
                            <h6 class="font-bold text-blue-600">Recent Orders</h6>
                        </div>
                        <div class="divide-y divide-gray-200">
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        @foreach ($recentOrders as $order)
                                            <tr>
                                                <td class="px-4 py-2 text-sm">
                                                    <a href="{{ route('admin.store.orders.show', $order->id) }}" class="text-blue-600 hover:underline">#{{ $order->id }}</a>
                                                </td>
                                                <td class="px-4 py-2 text-sm">{{ $order->user->firstname }} {{ $order->user->lastname }}</td>
                            ...
                            <div class="p-3 border-t">
                                <a href="{{ route('admin.store.orders.index', ['product' => $product->id]) }}"
                                    class="block w-full px-4 py-2 text-sm bg-gray-100 text-gray-700 rounded hover:bg-gray-200 text-center transition">
